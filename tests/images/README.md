...
Two corpora with opposite properties, which is why they are separate directories.

| | this directory | [`real/`](real/) |
|---|---|---|
| Origin | generated by `tools/make_test_images.py` | contributed, from real AI tools |
| On regeneration | rewritten byte-identically | never touched |
| Metadata | written by this project, well-formed | whatever the generator emitted |
| Expected verdict | known by construction | asserted by the contributor |
| In the sdist | yes | no — it grows without bound |

**Editing a file here is pointless** — the generator overwrites it. Change
`tools/make_test_images.py` instead. Editing a file in `real/` is worse than pointless:
it destroys the only reason that file exists.

24 generated images, plus [`manifest.json`](manifest.json) recording what each should
produce.
