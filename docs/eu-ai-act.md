...
**`USE_GIFSICLE_ENGINE` leaves GIFs without a visible mark.** That engine holds no image in
memory, so nothing can be composited onto it. The verdict is still detected and still
published on `/meta/` with `"labelled": false`, so the 50(5) accessible disclosure is intact
— but for those images it is the *only* disclosure, and the 50(4) visible one is absent.
Either turn the setting off or make the mark part of the surrounding page.

On the default PIL engine GIFs are read and labelled like any other format, every frame of
an animation included. See [GIFs](configuration.md#gifs).
